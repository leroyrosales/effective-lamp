...
def http_date(
    timestamp: t.Optional[t.Union[datetime,
                                  date, int, float, struct_time]] = None
) -> str:
    ...
    if isinstance(timestamp, date):
        if not isinstance(timestamp, datetime):
            ...
            timestamp = datetime.combine(
                timestamp, time(), tzinfo=timezone.utc)
        else:
...
def dump_cookie(
    ...
    if expires is not None:
        ...
        expires = http_date(datetime.now(
            tz=timezone.utc).timestamp() + max_age)
