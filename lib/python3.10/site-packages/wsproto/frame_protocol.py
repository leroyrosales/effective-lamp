...
class XorMaskerSimple:
    ...
    def process(self, data: bytes) -> bytes:
        if data:
            ...
            self._masking_key = (
                self._masking_key[key_rotation:] +
                self._masking_key[:key_rotation]
            )
...
class FrameDecoder:
    ...
    def process_buffer(self) -> Optional[Frame]:
        ...
        if finished:
            final = bytearray()
            for extension in self.extensions:
                result = extension.frame_inbound_complete(
                    self, self.header.fin)
        ...
        frame = Frame(self.effective_opcode, payload,
                      finished, self.header.fin)
    ...
    def parse_header(self) -> bool:
        ...
        payload_len = self.parse_extended_payload_length(
            opcode, payload_len_short)
...
class FrameProtocol:
    ...
    def _make_fin_rsv_opcode(self, fin: bool, rsv: RsvBits, opcode: Opcode) -> int:
        fin_bits = int(fin) << 7
        rsv_bits = (int(rsv.rsv1) << 6) + \
            (int(rsv.rsv2) << 5) + (int(rsv.rsv3) << 4)
