...
class Service(ABC):
    ...
    def __init__(
        ...
        self.log_file = open(
            os.devnull, "wb") if not _HAS_NATIVE_DEVNULL and log_file == DEVNULL else log_file
    ...
    def start(self) -> None:
        ...
        try:
            ...
            if "executable needs to be in PATH" in err.msg:
                logger.debug(
                    "driver not found in PATH, trying Selenium Manager")
                ...
                try:
                    ...
                    logger.debug(
                        "Unable to obtain driver using Selenium Manager: " + new_err.msg)
                    raise err
        ...
        while True:
            ...
            if count == 60:
                raise WebDriverException(
                    f"Can not connect to the Service {self.path}")
    ...
    def _start_process(self, path: str) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Started executable: `{self.path}` in a child process with pid: {self.process.pid}")
        except TypeError:
            raise
