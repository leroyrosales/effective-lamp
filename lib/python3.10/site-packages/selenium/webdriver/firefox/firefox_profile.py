...
class FirefoxProfile:
    ...
    def __init__(self, profile_directory=None):
        ...
        if not self.profile_dir:
            ...
            shutil.copytree(
                self.profile_dir, newprof, ignore=shutil.ignore_patterns(
                    "parent.lock", "lock", ".parentlock")
            )
            self.profile_dir = newprof
            os.chmod(self.profile_dir, 0o755)
            self._read_existing_userjs(
                os.path.join(self.profile_dir, "user.js"))
        ...
        try:
            with open(userjs, encoding="utf-8") as f:
                for usr in f:
                    ...
                    try:
                        self.default_preferences[matches.group(
                            1)] = json.loads(matches.group(2))
                    except Exception:
                        warnings.warn(
