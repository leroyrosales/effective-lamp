...
if TYPE_CHECKING:
    ...
    try:
        ...
        if sys.platform == "linux":
            ...
            def pidfd_open(fd: int, flags: int) -> int:
                result = _cdll_for_pidfd_open.syscall(
                    __NR_pidfd_open, fd, flags)
                if result < 0:
    ...
    for key in ("universal_newlines", "text", "encoding", "errors", "bufsize"):
        if options.get(key):
            raise TypeError(
                ...
                "unbuffered byte streams, the '{}' option is not supported".format(
                    key)
            )
...
async def _windows_deliver_cancel(p):
    try:
        ...
        warnings.warn(RuntimeWarning(
            f"TerminateProcess on {p!r} failed with: {exc!r}"))
