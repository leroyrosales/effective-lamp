...
if posix:
    def SLEEP(seconds): return ["/bin/sleep", str(seconds)]
else:
    def SLEEP(seconds): return python(
    ...
    async with background_process(
        ...
        async def expect(idx, request):
            async with _core.open_nursery() as nursery:
                ...
                nursery.start_soon(drain_one, proc.stderr,
                                   request * 2, idx * 2 + 1)
...
async def test_custom_deliver_cancel():
    ...
    async with _core.open_nursery() as nursery:
        nursery.start_soon(
            partial(run_process, SLEEP(9999),
                    deliver_cancel=custom_deliver_cancel)
        )
        await wait_all_tasks_blocked()
        nursery.cancel_scope.cancel()
